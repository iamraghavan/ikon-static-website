(function($) {
	...
	if($('.category-menu li.category-dropdown ul').length){
		$('.category-menu .category-list li.category-dropdown').append('<div class="dropdown-btn"><span class="icon-3"></span></div>');
	...
	if ($('.single-item-carousel').length) {
		$('.single-item-carousel').owlCarousel({
			...
			smartSpeed: 500,
			autoplay: 1000,
			navText: [ '<span class="icon-19"></span>', '<span class="icon-5"></span>' ],
			responsive:{
				0:{
					items:1
				},
	...
	if($('.curved-circle').length) {
        $('.curved-circle').circleType({position: 'absolute', dir: 1.9, radius: 90, forceHeight: true, forceWidth: true});
    }
